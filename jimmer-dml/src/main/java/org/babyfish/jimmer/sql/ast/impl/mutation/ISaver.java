...
public class ISaver {
    ...
    private ISaver(SaveContext ctx) {
        this.ctx = ctx;
    }

    @SuppressWarnings("unchecked")
    public <E> SimpleSaveResult<E> save(E entity) {
        ImmutableType immutableType = getType(entity);
    ...
    protected ImmutableType getType(Object entity) {
        ImmutableType immutableType;
        if (entity instanceof ImmutableSpi) {
            ImmutableSpi spi = (ImmutableSpi) entity;
            immutableType = spi.__type();
        } else {
            immutableType = ImmutableType.tryGet(entity.getClass());
            if (immutableType == null) {
                immutableType = ctx.path.getType();
            }
        }
    ...
    private boolean saveSelf(PreHandler preHandler) {
        IOperator operator = new IOperator(ctx);
